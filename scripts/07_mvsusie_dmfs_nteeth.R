...
output_rds <- file.path(results_dir, "dmfs_nteeth_mvsusie_results.RDS")
output_csv <- file.path(results_dir, "dmfs_nteeth_mvsusie_summary.csv")
output_log <- file.path(results_dir, "dmfs_nteeth_mvsusie_log.txt")
...
run_mvsusie_locus <- function(prefix, ld_dir) {
  ...
  if (nrow(snp_map) < 2) {
    cat("Skipping", prefix, "- fewer than 2 SNPs shared across LD, DMFS, and Nteeth\n")
    return(NULL)
  }
  ...
  if (any(is.na(dmfs_locus$Effect)) || any(is.na(nteeth_locus$Effect))) {
...
if (length(all_summaries) > 0) {
  ...
  final_summary <- final_summary %>%
    arrange(desc(PIP), desc(abs(Z_DMFS)), desc(abs(Z_Nteeth)))
