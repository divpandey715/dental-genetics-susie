...
output_rds <- file.path(results_dir, "dmfs_nteeth_dfss_mvsusie_results.RDS")
output_csv <- file.path(results_dir, "dmfs_nteeth_dfss_mvsusie_summary.csv")
output_log <- file.path(results_dir, "dmfs_nteeth_dfss_mvsusie_log.txt")
...
############################
# Small helper for p-value columns
############################
...
run_mvsusie_locus <- function(prefix, ld_dir) {
  ...
  ############################
  # Keep SNPs present in all three GWAS files
  ############################
  
  snp_map <- snp_map[
    MarkerName %in% dmfs$MarkerName &
      MarkerName %in% nteeth$MarkerName &
  ...
  if (nrow(snp_map) < 2) {
    cat("Skipping", prefix, "- fewer than 2 SNPs shared across LD, DMFS, Nteeth, and DFSS\n")
    return(NULL)
  }
  ...
  nteeth_locus <- nteeth[J(snp_map$MarkerName)]
  dfss_locus <- dfss[J(snp_map$MarkerName)]
  
  if (
    any(is.na(dmfs_locus$Effect)) ||
    any(is.na(nteeth_locus$Effect)) ||
    any(is.na(dfss_locus$Effect))
  ) {
  ...
  if (
    any(is.na(dmfs_locus$StdErr)) ||
    any(is.na(nteeth_locus$StdErr)) ||
    any(is.na(dfss_locus$StdErr))
  ) {
    cat("Skipping", prefix, "- missing standard errors after alignment\n")
    return(NULL)
  }
  ...
  Z <- cbind(
    ...
    Nteeth = nteeth_locus$Effect / nteeth_locus$StdErr,
  ...
  N_vec <- c(
    ...
    Nteeth = median(nteeth_locus$N, na.rm = TRUE),
  ...
  locus_summary <- data.frame(
    ...
    Pvalue_DMFS = get_pvalue(dmfs_locus),
    N_DMFS = dmfs_locus$N,
    
    Effect_Nteeth = nteeth_locus$Effect,
    StdErr_Nteeth = nteeth_locus$StdErr,
    Pvalue_Nteeth = get_pvalue(nteeth_locus),
    N_Nteeth = nteeth_locus$N,
    
    Effect_DFSS = dfss_locus$Effect,
    StdErr_DFSS = dfss_locus$StdErr,
    Pvalue_DFSS = get_pvalue(dfss_locus),
    N_DFSS = dfss_locus$N
  )
...
get_ld_prefixes <- function(ld_dir) {
  if (!dir.exists(ld_dir)) {
    return(character(0))
  }
  
  ld_files <- list.files(ld_dir, pattern = "\\.ld$", full.names = FALSE)
  sub("\\.ld$", "", ld_files)
}

dmfs_prefixes <- get_ld_prefixes(dmfs_ld_dir)
nteeth_prefixes <- get_ld_prefixes(nteeth_ld_dir)
dfss_prefixes <- get_ld_prefixes(dfss_ld_dir)
...
for (prefix in all_prefixes) {
  ...
  if (prefix %in% dfss_prefixes) {
    ld_dir <- dfss_ld_dir
  } else if (prefix %in% nteeth_prefixes) {
    ld_dir <- nteeth_ld_dir
  } else if (prefix %in% dmfs_prefixes) {
    ld_dir <- dmfs_ld_dir
  } else {
    cat("Skipping", prefix, "- not found in DMFS, Nteeth, or DFSS LD folders\n")
...
if (length(all_summaries) > 0) {
  ...
  final_summary <- final_summary %>%
    arrange(desc(PIP), desc(abs(Z_DMFS)), desc(abs(Z_Nteeth)), desc(abs(Z_DFSS)))
