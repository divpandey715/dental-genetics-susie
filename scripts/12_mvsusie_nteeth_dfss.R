...
output_rds <- file.path(results_dir, "nteeth_dfss_mvsusie_results.RDS")
output_csv <- file.path(results_dir, "nteeth_dfss_mvsusie_summary.csv")
output_log <- file.path(results_dir, "nteeth_dfss_mvsusie_log.txt")
...
run_mvsusie_locus <- function(prefix, ld_dir) {
  ...
  if (nrow(snp_map) < 2) {
    cat("Skipping", prefix, "- fewer than 2 SNPs shared across LD, DFSS, and Nteeth\n")
    return(NULL)
  }
  ...
  if (any(is.na(dfss_locus$Effect)) || any(is.na(nteeth_locus$Effect))) {
  ...
  locus_summary <- data.frame(
    ...
    Pvalue_Nteeth = nteeth_locus$P.value,
...
nteeth_ld_files <- list.files(nteeth_ld_dir, pattern = "\\.ld$", full.names = FALSE)

dfss_prefixes <- sub("\\.ld$", "", dfss_ld_files)
nteeth_prefixes <- sub("\\.ld$", "", nteeth_ld_files)
...
if (length(all_summaries) > 0) {
  ...
  final_summary <- final_summary %>%
    arrange(desc(PIP), desc(abs(Z_DFSS)), desc(abs(Z_Nteeth)))
