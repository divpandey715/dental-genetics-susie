...
output_file <- file.path(
  root,
  "results/dfss_susie_results.RDS"
)
...
for (i in seq_along(ld_files)) {
  ...
  if (nrow(R_raw) != length(snp_names)) {
    ...
    warning(
      "LD matrix dimensions do not match SNP list, skipping ",
      locus_id
    )
  ...
  chr_pos_names <-
    id_lookup$coords[matched_indices]
  ...
  if (length(common_snps) < 10) {
    ...
    warning(
      "Too few overlapping SNPs, skipping ",
      locus_id
    )
  ...
  if (nrow(R) < 10) {
    ...
    warning(
      "Too few SNPs after removing NA LD rows/columns, skipping ",
      locus_id
    )
